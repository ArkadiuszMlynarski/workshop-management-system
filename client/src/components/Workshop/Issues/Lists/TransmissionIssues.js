...
class TransmissionIssues extends Component {
  componentDidMount() {
  ...
  render() {
    ...
    let transmissionIssues = [];
    for (let i = 0; i < issuess.length; i++) {
      if (issuess[i].props.issue.type === "TRANSMISSION") {
        transmissionIssues.push(issuess[i]);
      }
    }
